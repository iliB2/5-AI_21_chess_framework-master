...
class EvaluateUtility(Utility):
    ...
    def getPieceValue(self, board: chess.Board, x, y):
        if board.pieces(piece_type=chess.PAWN, color=chess.WHITE) | board.pieces(piece_type=chess.ROOK, color=chess.WHITE | board.pieces(piece_type=chess.BISHOP, color=chess.WHITE) | board.pieces(piece_type=chess.QUEEN, color=chess.WHITE) | board.pieces(piece_type=chess.KING, color=chess.WHITE)):
            absolutevalue = self.getAbsoluteValue(board, x, y)
        elif board.pieces(piece_type=chess.PAWN, color=chess.BLACK) | board.pieces(piece_type=chess.ROOK, color=chess.BLACK | board.pieces(piece_type=chess.BISHOP, color=chess.BLACK) | board.pieces(piece_type=chess.QUEEN, color=chess.BLACK) | board.pieces(piece_type=chess.KING, color=chess.BLACK)):
            absolutevalue = self.getAbsoluteValue(board, x, y)
